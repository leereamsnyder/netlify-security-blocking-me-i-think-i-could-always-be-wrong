...
test.describe('Build-time generated social media (og:image) images', () => {
	test('blog article should have og:image metadata', async ({ page }) => {
		// worth calling out that I only generate the last 4–5 years of articles
		// so this won't last forever…
		await page.goto('/row-stack-space-layout-components')
		const metaTag = await page.locator('meta[property="og:image"]')
		await expect(metaTag).toHaveAttribute(
			'content',
			// full URL for the public site, matching the pattern for the article
			'https://www.leereamsnyder.com/og-image-generator/row-stack-space-layout-components/og-image.jpg',
		)
	...
	test('should be able to load a pre-generated image', async ({ page, context }) => {
		...
		await page.goto('/og-image-generator/row-stack-space-layout-components/og-image.jpg')
	})

	test('should NOT find an og:image tag for a very old article', async ({ page }) => {
		await page.goto('/50-jokes-about-state-flags')
	...
	test('og image URLs with /blog/ should redirect to version without /blog/', async ({ page }) => {
		await page.goto('/og-image-generator/blog/take-the-dang-picture/og-image.jpg')
		await page.waitForURL('/og-image-generator/take-the-dang-picture/og-image.jpg')
	})
})
